...
def plot_training_results(hist_input, plot_name: str, is_frozen_layers) -> None:
    ...
    plt.savefig("./drive/My Drive/Thesis/results/dataset-{}_model-{}_{}-Loss.png".format(config.dataset, config.model, plot_name))
    plt.show()
    ...
    plt.ylim(0, 1.1)
    plt.legend(loc="upper right")
    plt.savefig("./drive/My Drive/Thesis/results/dataset-{}_model-{}_{}-Accuracy.png".format(config.dataset, config.model, plot_name))
    plt.show()
