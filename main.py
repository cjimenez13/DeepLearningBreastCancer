...
from data_operations.data_preprocessing import calculate_class_weights, dataset_stratified_split, \
    import_cbisddsm_testing_dataset, import_cbisddsm_training_dataset, import_minimias_dataset, import_cbisddsm_dataset,import_cbisddsm_dataset_gan
...
def main() -> None:
    ...
    if config.run_mode == "train":
        ...
        # Binary classification (CBIS-DDSM dataset).
            ...
            images, labels = import_cbisddsm_dataset(l_e)
            #images_gan, labels_gan = import_cbisddsm_dataset_gan(l_e)

            # Split training dataset into training/validation sets (75%/25% split).
            X_train, X_val, y_train, y_val = dataset_stratified_split(split=0.15, dataset=images, labels=labels)
            #X_train = np.concatenate((X_train,images_gan), axis=0)
            #y_train = np.concatenate((y_train,labels_gan), axis=0)
            #X_train, y_train = generate_image_transforms(X_train, y_train)
            # Calculate class weights.
            #X_train, y_train = load_training()
            #X_val, y_val = load_testing()
            class_weights = calculate_class_weights(y_train, l_e)
        ...
        print_cli_arguments()
        #y_val = np.array([remap_label(l) for l in y_val])
        ...
        if config.dataset == "mini-MIAS":
            ...
            model.make_prediction(X_val)
            model.evaluate_model(X_val, y_val, l_e, 'B-M', runtime)
            #model.evaluate_model(y_val, l_e, 'B-M', runtime)
    ...
    # Run in testing mode.
        ...
        if config.dataset == "mini-MIAS":
            images, labels = import_minimias_dataset(data_dir="../data/{}/images_processed".format(config.dataset),
                                                     label_encoder=l_e)
            _, X_test, _, y_test = dataset_stratified_split(
                split=0.20, dataset=images, labels=labels)
...
def parse_command_line_arguments() -> None:
    ...
    parser.add_argument("-at", "--auto_augment",
                        default=True,
                        help="Apply auto augment algorithm"
                        )
    ...
    config.auto_augment = args.auto_augment
