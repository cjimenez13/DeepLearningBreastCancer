...
def generate_image_transforms(images, labels):
    ...
    #class_balance = [np.ones(labels.shape[0]), np.ones(labels.shape[0])]
    #max_count = labels.shape[0] * augmentation_multiplier  # Balance classes.
    #to_add = [max_count - i for i in class_balance]
    to_add = labels.shape[0]-1
    print(to_add)
    
    #to_add = len(labels[0]) * augmentation_multiplier

    for i in range(to_add):
        transformed_image = create_individual_transform(images[i],available_transforms)
        images_with_transforms = np.append(images_with_transforms, [transformed_image], axis=0)
        labels_with_transforms = np.append(labels_with_transforms, [labels[i]], axis=0)
        transformed_image = create_individual_transform(images[i+1],available_transforms)
        images_with_transforms = np.append(images_with_transforms, [transformed_image], axis=0)
        labels_with_transforms = np.append(labels_with_transforms, [labels[i+1]], axis=0)
        transformed_image = create_individual_transform(images[i-1],available_transforms)
        images_with_transforms = np.append(images_with_transforms, [transformed_image], axis=0)
        labels_with_transforms = np.append(labels_with_transforms, [labels[i-1]], axis=0)
        # # if int(to_add[i]) == 0:
        ...
        #     # elif config.model == "VGG" or config.model == "Inception" or config.model == "VGG16":
    ...
    if config.dataset == "mini-MIAS":
        ...
        for y_val in y_vals:
            for i in range(num_classes):
                counts[i] += y_val[i]
    elif config.dataset == "mini-MIAS-binary" or config.dataset == "CBIS-DDSM":
        num_classes = 2
        counts = np.zeros(num_classes)
        for y_val in y_vals:
            if y_val == 0:
