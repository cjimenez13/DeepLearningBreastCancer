...
def import_cbisddsm_dataset(label_encoder):
    ...
    #testPath = "./drive/My Drive/Thesis/data/CBIS-DDSM/testing_png.csv"
    # trainingPath = "../data/CBIS-DDSM/training_png.csv"
    # testPath = "../data/CBIS-DDSM/testing_png.csv"
    ...
    return images, labels
def import_cbisddsm_dataset_gan(label_encoder):
    print("Importing CBIS-DDSM training set")
    trainingPath = "only_gan_all_png.csv"
    df = pd.read_csv(trainingPath)
    list_IDs = df['img_path'].values
    labels = encode_labels(df['label'].values, label_encoder)
    images = list()
...
def preprocess_image(image_path: str) -> np.ndarray:
    ...
    if not config.is_roi:
        if config.model == "VGG" or config.model == "Inception" or config.model == "DenseNet" or config.model == "VGG16":
...
def img_to_array_d(img, data_format='channels_last', dtype='float32'):
