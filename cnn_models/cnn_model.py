...
from cnn_models.auto_augment_generator import *
import os
...
class CnnModel:
    ...
    def train_model(self, X_train, X_val, y_train, y_val, class_weights) -> None:
        ...
        if not self.model_name == "CNN":
            ...
            self.fit_model(X_train, X_val, y_train, y_val,
                           class_weights, is_frozen_layers=True)

            # Plot the training loss and accuracy.
            plot_training_results(
                self.history, "Initial_training", is_frozen_layers=True)
            ...
            if config.verbose_mode:
                print(
                    "Unfreezing '{}' layers (all layers now unfrozen)".format(layer_name))
            ...
            self.fit_model(X_train, X_val, y_train, y_val,
                           class_weights, is_frozen_layers=False)

            # Plot the training loss and accuracy.
            plot_training_results(
                self.history, "Fine_tuning_training", is_frozen_layers=False)
        ...
        # Small CNN (no transfer learning).
            ...
            self.fit_model(X_train, X_val, y_train, y_val,
                           class_weights, is_frozen_layers=True)
            plot_training_results(
                self.history, "Initial_training", is_frozen_layers=False)
    ...
    def compile_model(self, learning_rate) -> None:
        ...
        if config.dataset == "CBIS-DDSM" or config.dataset == "mini-MIAS-binary":
            self._model.compile(optimizer=RMSprop(learning_rate),
    ...
    def fit_model(self, X_train, X_val, y_train, y_val, class_weights, is_frozen_layers: bool) -> None:
        ...
        # # Remap labels
        # y_train = np.array([self.remap_label(l) for l in y_train])
        # y_val = np.array([self.remap_label(l) for l in y_val])

        # # Create a new dimension for color in the images arrays
        # X_train = X_train.reshape((n_train_img, img_w, img_h, 1))
        # X_val = X_val.reshape((n_test_img, img_w, img_h, 1))

        # # Convert from 16-bit (0-65535) to to 8-bit (0-255)
        # X_train = X_train.astype('uint16') / 256
        # X_val = X_val.astype('uint16') / 256
        ...
        # train_generator = train_datagen.flow(X_train, y_train, batch_size=config.batch_size, save_to_dir="../data", save_prefix='', save_format='png')
        # validation_generator = train_datagen.flow(X_val, y_val, batch_size=config.batch_size, save_to_dir="../data", save_prefix='', save_format='png')
        train_generator = train_datagen.flow(X_train, y_train, batch_size=config.batch_size)
        validation_generator = train_datagen.flow(X_val, y_val, batch_size=config.batch_size)

        # Early stopping (stop training after the validation loss reaches the minimum)
        earlystopping = EarlyStopping(monitor='val_loss', mode='min', patience=30, verbose=1)

        # Callback for checkpointing
        checkpoint = ModelCheckpoint(config.name+'.h5',
    ...
    def make_prediction(self, x):
        ...
        if config.dataset == "mini-MIAS" or config.dataset == "mini-MIAS-binary":
            self.prediction = self._model.predict(
                x=x.astype("float32"), batch_size=config.batch_size)
        elif config.dataset == "CBIS-DDSM":
            self.prediction = self._model.predict(x=x)
        # print(self.prediction)

    def evaluate_model(self, X_val, y_true: list, label_encoder: LabelEncoder, classification_type: str, runtime) -> None:
        """
        ...
        tra_loss_fe = self.history.history['loss']
        tra_acc_fe = self.history.history['binary_accuracy']
        val_loss_fe = self.history.history['val_loss']
        val_acc_fe = self.history.history['val_binary_accuracy']
        ...
        opt_model = models.load_model(config.name+'.h5')
        test_loss_fe, test_acc_fe = self._model.evaluate(X_val, y_val, verbose=False)
        opt_test_loss_fe, opt_test_acc_fe = opt_model.evaluate(X_val, y_val, verbose=False)
        ...
        #self.make_prediction(X_val)
        #self.evaluate_model(y_val, l_e, 'B-M', runtime)


        # # Compare results with other similar papers' result.
        # with open(
        #         './other_paper_results.json') as config_file:  # Load other papers' results from JSON.
    ...
    def save_model(self) -> None:
        ...
        # Scratch space
        self._model.save(
            "./dataset-{}_mammogramtype-{}_model-{}_lr-{}_b-{}_e1-{}_e2-{}_roi-{}_{}_saved-model.h5".format(
                config.dataset,
    ...
    def save_weights(self) -> None:
        ...
        if self.model_name == "VGG-common" or self.model_name == "MobileNet":
            ...
            self._model.save_weights(
                "./dataset-{}_mammogramtype-{}_model-{}_lr-{}_b-{}_e1-{}_e2-{}_roi-{}_{}_all_weights.h5".format(
                    config.dataset,
                    config.mammogram_type,
                    config.model,
                    config.learning_rate,
            ...
            np.save(
                "./dataset-{}_mammogramtype-{}_model-{}_lr-{}_b-{}_e1-{}_e2-{}_roi-{}_{}_fc_weights.npy".format(
                    config.dataset,
                    config.mammogram_type,
                    config.model,
                    config.learning_rate,
    ...
    def load_minimias_weights(self) -> None:
        ...
        self._model.load_weights(
            "./dataset-mini-MIAS-binary_mammogramtype-all_model-MobileNet_lr-0.0001_b-2_e1-150_e2-50_roi-False__all_weights.h5"
        )
        # self._model.load_weights(
    ...
    def load_minimias_fc_weights(self) -> None:
        ...
        weights = np.load(
            "./dataset-mini-MIAS-binary_mammogramtype-all_model-MobileNet_lr-0.0001_b-2_e1-150_e2-50_roi-False__fc_weights.npy"
        )
        # weights = np.load(
