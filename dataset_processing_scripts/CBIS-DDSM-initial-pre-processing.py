...
if __name__ == '__main__':
  ...
  for index in range(len(list_IDs)):
    img_path = "."+list_IDs[index]
    print(img_path)
    img_path_result = img_path.replace('.dcm','.png')
    img_path_result = img_path_result.replace('/tmp/','/pre/')
    print(img_path_result)

    fileName = os.path.split(img_path_result)[1]
    basePath = os.path.split(img_path_result)[0]
    print(fileName)
    print(basePath)
    ...
    image = tf.image.resize_with_pad(decoded_png, 1024, 1024)  # or 'tf.image.resize'
    image /= 255
    tf.keras.preprocessing.image.save_img(img_path_result, image)




    # ds = pydicom.read_file(img_path) # read dicom image
    # img = ds.pixel_array # get image array
    # cv2.imwrite(img_path_result,img) # write png image
    
    
    


    #os.remove(img_path.replace('.dcm','.png'))
